...
public:
   ...
   */
  bool Contains(const Comparable &x) const { return _contains(x, _root); }
   ...
   */
  void Clear() { _makeEmpty(_root); }

  /**
   * Insert x into the tree; duplicates are ignored.
   */
  void Insert(const Comparable &x) { _insert(x, _root); }

  /**
   * Insert x into the tree; duplicates are ignored.
   */
  void Insert(Comparable &&x) { _insert(std::move(x), _root); }

  /**
   * Remove x from the tree. Nothing is done if x is not found.
   */
  void Remove(const Comparable &x) { _remove(x, _root); }
...
private:
  ...
  **/
  ...
  void _printTreeInorder(BinaryNode *t, ostream &out) const {
    ...
    _printTreeInorder(t->right, out << t->element << "");
  }
  ...
  void _printTreePreOrder(BinaryNode *t, ostream &out) const {
    if (t == NULL)
      return;
    _printTreePreOrder(t->left, out << t->element << "");
